// apps/frontend/src/components/customer/BookingSuccess.tsx (continued)
'use client';
...
export const BookingSuccess: React.FC = () => {
  ...
  const { data: bookingData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const booking = await apiClient.get(`/api/bookings/${bookingId}`);
  ...
  const formatDateTime = (): string => {
    ...
    const date = new Date(bookingData.scheduledTime);
    ...
    return `${dateLabel} - ${date.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}`;
  };
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600 mb-4">
            {language === 'ta' ? 'பதிவு எண் இல்லை' : 'No booking ID found'}
          </p>
  ...
  if (error || !bookingData) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600 mb-4">
            {language === 'ta' ? 'பதிவு தகவல் கிடைக்கவில்லை' : 'Booking details not found'}
          </p>
          <Button onClick={() => router.push('/')}>
            {language === 'ta' ? 'முகப்புக்கு செல்' : 'Go Home'}
          </Button>
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-blue-50">
      <div className="max-w-2xl mx-auto p-4">
        ...
        <div className="text-center pt-8 mb-8">
          ...
          <h1 className="text-4xl font-black mb-4 text-green-700">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium">
                  {bookingData.contactInfo?.name || 'N/A'}
                </span>
              </div>
              <div className="flex items-center gap-3">
                <Phone className="w-4 h-4 text-gray-500" />
                <span>{bookingData.contactInfo?.phone || 'N/A'}</span>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium">
                  {getServiceTitle()} - ₹{bookingData.totalCost || 'TBD'}
                </span>
              </div>
            </div>
          </div>
